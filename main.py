...
def print_summary():
    ...
    print("\nResults summary")
    print(f"  {'Approach':<35} {'AUC-ROC':>8}  {'F1':>6}")
    ...
    for folder, label in metrics_files:
        ...
        if aucs and f1s:
            ...
            print(f"  {label:<35} {best_auc:>8.4f}  {best_f1:>6.4f}")
...
def main():
    ...
    parser.add_argument("--models", nargs="?", const="all", default=None,
        choices=["all", "baselines", "baseline", "tfidf", "crossval"],
        help="Train models. Options: all (default), baselines, baseline, tfidf, crossval")
